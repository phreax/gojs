...
GameStore.prototype = {
  ...
  createGame: function(cb) {
    var attr = {nplayers: 0, player1: null, player2: null},
        client = this.client,
        prefix = this.prefix,
        key    = this.key;
  ...
  readGame: function(gameID,cb) {
    ...
    client.hgetall(key(prefix,'games',gameID),function(err,ret) {
      if(err || ret && isEmpty(ret)) {
        cb(err);
      } else {
        cb(null,ret);
      }
  ...
  readAllGames: function(cb) {
    ...
    var collection = [];
    client.smembers(key(prefix,'keys','games'),function(err,keys) {
      if(err) {
        cb(err,keys);
      } else {
        keys.forEach(function(id) {
          client.hgetall(key(prefix,'games',id),function(err,model) {
            if(err || !model) {
              cb(err,ret);
            } else {
